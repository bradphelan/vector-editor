<script>
  ...
  function updateCppInitializerList() {
    cppInitializerList = 'std::vector<std::vector<cadcam::Point2D>> curves = {\n';
    for (let curve of curves) {
      cppInitializerList += '  {';
...
<div class="container">
  <div class="canvas-area">
    <canvas id="canvas" on:mousedown={handleMouseDown} on:mousemove={handleMouseMove} on:mouseup={handleMouseUp} on:mouseout={handleMouseOut} on:dblclick={handleDoubleClick}></canvas>
  </div>
  <div class="textarea-area">
    <textarea readonly value={cppInitializerList} rows="10" cols="50"></textarea>
...
<div class="button-area">
  <button on:click={() => changeMode('freehand')} class="{mode === 'freehand' ? 'active' : ''}">Freehand</button>
  <button on:click={() => changeMode('lines')} class="{mode === 'lines' ? 'active' : ''}">Lines</button>
